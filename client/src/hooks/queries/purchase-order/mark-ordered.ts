...
export function useMarkPurchaseOrderOrdered({ purchaseOrderId }: Args) {
  const queryClient = useQueryClient();

  return useMutation<unknown, Error>({
    mutationKey: ["purchase-order", purchaseOrderId, "mark-ordered"],
    mutationFn: () =>
      api
        .post(`/api/purchase-order/${purchaseOrderId}/mark-ordered`)
        .then((resp) => resp.data),
    onSuccess: () => {
